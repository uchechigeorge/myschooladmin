...
import { editGeneralFeesModalID } from 'src/app/models/components-id';
...
const LOADER_ID = "update-gfees-loader";


@Component({
  selector: 'app-edit-general-fees',
  templateUrl: './edit-general-fees.component.html',
  styleUrls: ['./edit-general-fees.component.scss'],
})
export class EditGeneralFeesComponent implements OnInit {
  ...
  public details: IEditInput[] = [
    {
      ...
      updateInput: async () => {
        ...
        let value = await this.paymentService.updateGeneralFeesTitle({ generalFeesId: this.feesId, feesTitle: this.getInput(EditInputs.Title).model?.trim() })
        .toPromise()
        .then(res => {
    ...
    },
    {
      ...
      updateInput: async () => {
        ...
        await this.showLoading();
        let value = await this.paymentService.updateGeneralFeesAmount({ generalFeesId: this.feesId, feesAmount: parseFloat(amount).toString() })
        .toPromise()
        .then(res => {
          return this.proceed(res);
        }, (err) => {
    ...
    },
    {
      ...
      updateInput: async () => {
        await this.showLoading();
        let value = await this.paymentService.updateGeneralFeesDescription({ generalFeesId: this.feesId, feesDescription: this.getInput(EditInputs.Description).model?.trim() })
        .toPromise()
        .then(res => {
          return this.proceed(res);
        }, (err) => {
  ...
  getFees() {
    this.paymentService.viewGeneralFees({
      updateType: "2",
      qString: this.feesId,
    })
    .subscribe(async (res) => {
      if(res.statuscode == 200){
        ...
        this.className = response.subClassName;
  ...
  delete() {
    ...
    this.paymentService.deleteGeneralFee({ generalFeeId: this.feesId })
    .subscribe(async (res) => {
      if(res.statuscode == 200) {

        await this.paymentService.saveCredentials({
          adminId: res.dataResponse.adminid,
          token: res.dataResponse.token
        });
  ...
  dismissModal() {
    this.modalCtrl.dismiss('', '', editGeneralFeesModalID);
  }
}
